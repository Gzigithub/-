...
def f1(driver, num):
    ...
    if num != int(cnum):
        ...
        locator = (By.XPATH, "(//a[@id='title-list'])[1][string()!='%s']" % val)
...
def work(conp):
    gg_meta(conp,data=data,diqu="海南省东方市")
...
if __name__ == '__main__':
    conp=["postgres","since2015","192.168.3.171","hainan","dongfang"]

    work(conp=conp)

    # driver=webdriver.Chrome()
    # url="http://zw.hainan.gov.cn/ggzy/dfggzy/GGjxzbgs1/index.jhtml"
    # driver.get(url)
    # df = f2(driver)
    # print(df)
    # for i in range(1, 5):
