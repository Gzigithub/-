    ...
    if num != int(cnum):
        while True:
            ...
            if url_1 != url:
                ...
                # WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator)).click()
            if num > int(cnum):
                locator = (By.XPATH, "//a[@id='ContentPlaceHolder1_lbtnDown']")
                WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator)).click()
            else:
                locator = (By.XPATH, "//a[@id='ContentPlaceHolder1_lbntUp']")
                WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator)).click()
...
def work(conp, i=-1):
    data = [
        ["gcjs_gg","http://hzsggzyjy.gov.cn/cityInfoList.aspx?s=1&t=1",
         ["name", "ggstart_time", "href"]],

        ["qsydw_gg", "http://hzsggzyjy.gov.cn/cityInfoList.aspx?s=1&t=5",
         ["name", "ggstart_time", "href"]],

        ["qita_gg", "http://hzsggzyjy.gov.cn/cityInfoList.aspx?s=1&t=6",
         ["name", "ggstart_time", "href"]],
