...
def f2(driver):
    ...
    total_page = driver.find_element_by_xpath(
        "//div[@data-role='tab-content' and @class='']//ul[@class='m-pagination-page']/li[last()]/a").text
    driver.quit()
...
def switch(driver, ggtype, xmtype):
    ...
    val = driver.find_element_by_xpath(
        "//div[@data-role='tab-content' and @class='']//ul[@class='m-pagination-page']/li[last()]").text
    if ggtype != "建设工程":
        driver.find_element_by_xpath("//ul[@class='clearfix']/li[contains(string(),'%s')]" % ggtype).click()
        locator = (By.XPATH,
                   "//div[@data-role='tab-content' and @class='']//ul[@class='m-pagination-page']/li[last()][not(contains(string(),'%s'))]" % val)
        WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
    val = driver.find_element_by_xpath(
        "//div[@data-role='tab-content' and @class='']//ul[@class='m-pagination-page']/li[last()]").text

    driver.find_element_by_xpath(
        "//div[@data-role='tab-content' and @class='']/div[3]/a[contains(string(),'%s')]" % xmtype).click()
    locator = (By.XPATH,
               "//div[@data-role='tab-content' and @class='']//ul[@class='m-pagination-page']/li[last()][not(contains(string(),'%s'))]" % val)
    WebDriverWait(driver, 30).until(EC.visibility_of_element_located(locator))


    xxtype_signal = driver.find_element_by_xpath(
        "//div[@data-role='tab-content' and @class='']/div[3]/a[contains(string(),'%s')]" % xmtype).get_attribute(
        "value")
...
data = [
    ...
    ["gcjs_zhongbiao_gg",
     "http://ggzy.tlzw.gov.cn/tlsggzy/jsgc/subpage.html",
     ["name", "ggstart_time", "href", "info"], before(f1, "建设工程", "中标公示"), before(f2, "建设工程", "中标公示")],
