...
def f1(driver, num):
    ...
    # return
    if int(cnum) != int(num):
        url = "/".join(driver.current_url.split('/')[:-1])+'/index_'+str(num)+".htm"
        driver.get(url)
...
def f2(driver):
    try:
        ...
        WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
        driver.find_element_by_xpath("//div[@class='pagination']/a[contains(string(),'尾页')]").click()
        total_page = driver.find_element_by_xpath("//a[@class='current']").text.strip()
    except:
        total_page = 1
    driver.quit()
