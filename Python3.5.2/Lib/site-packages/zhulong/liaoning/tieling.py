import re

import requests
from bs4 import BeautifulSoup
from lmfscrap import web
from lxml import etree
from selenium import webdriver
from selenium.webdriver.common.by import By
from selenium.webdriver.support.wait import WebDriverWait
from selenium.webdriver.support import expected_conditions as EC
import pandas as pd
from zhulong.util.etl import est_html, est_meta
import time

_name_='tieling'
def f1(driver, num):
    if "http://tlggzyjy.com" in driver.current_url:
        cnum = driver.current_url.split('?')[0].split('/')[-1]
        # print("cnum",cnum)
        locator = (By.XPATH, "//div[@class='page_r_list']/ul/li/a")
        WebDriverWait(driver, 20).until(EC.visibility_of_all_elements_located(locator))
        val = driver.find_element_by_xpath("//div[@class='page_r_list']/ul/li/a").get_attribute("href")[-40:]
        # print(val)
        if int(cnum) != int(num):
            url = '/'.join(driver.current_url.split('?')[0].split('/')[:-1])+'/'+ str(num) + '?' + driver.current_url.split('?')[1]
            # print(url)
            driver.get(url)
            locator = (
                By.XPATH, '//div[@class="page_r_list"]/ul/li/a[not(contains(@href,"%s"))]' % val)
            WebDriverWait(driver, 20).until(EC.presence_of_element_located(locator))
        data = []
        page = driver.page_source
        body = etree.HTML(page)
        content_list = body.xpath("//div[@class='page_r_list']/ul/li")
        for content in content_list:
            name = content.xpath("./a/text()")[0].strip()
            ggstart_time = '-'.join(content.xpath("./span/text()")[0].split(' ')[0].split("/"))
            url = "http://tlggzyjy.com" + content.xpath("./a/@href")[0].strip()
            temp = [name, ggstart_time, url]
            data.append(temp)
            # print(temp)
        df = pd.DataFrame(data=data)
        df["info"] = None
        return df
    else:
        if "00300100" in driver.current_url:
            locator = (By.XPATH, '//*[@id="zbggmore2_DataGrid1"]/tbody/tr[1]/td[2]/a')
            WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
            val = driver.find_element_by_xpath('//*[@id="zbggmore2_DataGrid1"]/tbody/tr[1]/td[2]/a').text
            locator = (By.ID, "zbggmore2_Pager")
            WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
            cnum = driver.find_element_by_xpath('//*[@id="zbggmore2_Pager"]/table/tbody/tr/td[1]/font[3]/b').text
            if int(cnum) != int(num):
                driver.execute_script("javascript:__doPostBack('zbggmore2$Pager','{}')".format(num))
                locator = (By.XPATH, '//*[@id="zbggmore2_DataGrid1"]/tbody/tr[1]/td[2]/a')
                WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
                locator = (
                    By.XPATH, "//*[@id='zbggmore2_DataGrid1']/tbody/tr[1]/td[2]/a[not(contains(string(),'%s'))]" % val)
                WebDriverWait(driver, 20).until(EC.presence_of_element_located(locator))
            page = driver.page_source
            body = etree.HTML(page)
            content_list = body.xpath('//*[@id="zbggmore2_DataGrid1"]/tbody/tr')
        elif "00300200" in driver.current_url:
            locator = (By.XPATH, '//*[@id="zbgsmore2_DataGrid1"]/tbody/tr[1]/td[2]/a')
            WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
            val = driver.find_element_by_xpath('//*[@id="zbgsmore2_DataGrid1"]/tbody/tr[1]/td[2]/a').text
            locator = (By.ID, "zbgsmore2_Pager")
            WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
            cnum = driver.find_element_by_xpath('//*[@id="zbgsmore2_Pager"]/table/tbody/tr/td[1]/font[3]/b').text
            if int(cnum) != int(num):
                driver.execute_script("javascript:__doPostBack('zbgsmore2$Pager','{}')".format(num))
                locator = (By.XPATH, '//*[@id="zbgsmore2_DataGrid1"]/tbody/tr[1]/td[2]/a')
                WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
                locator = (
                    By.XPATH, "//*[@id='zbgsmore2_DataGrid1']/tbody/tr[1]/td[2]/a[not(contains(string(),'%s'))]" % val)
                WebDriverWait(driver, 20).until(EC.presence_of_element_located(locator))
            page = driver.page_source
            body = etree.HTML(page)
            content_list = body.xpath('//*[@id="zbgsmore2_DataGrid1"]/tbody/tr')
        elif "00300300" in driver.current_url:

            locator = (By.XPATH, '//*[@id="zbjgmore2_DataGrid1"]/tbody/tr[1]/td[2]/a')
            WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
            val = driver.find_element_by_xpath('//*[@id="zbjgmore2_DataGrid1"]/tbody/tr[1]/td[2]/a').text
            locator = (By.ID, "zbjgmore2_Pager")
            WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
            cnum = driver.find_element_by_xpath('//*[@id="zbjgmore2_Pager"]/table/tbody/tr/td[1]/font[3]/b').text
            if int(cnum) != int(num):
                driver.execute_script("javascript:__doPostBack('zbjgmore2$Pager','{}')".format(num))
                locator = (By.XPATH, '//*[@id="zbjgmore2_DataGrid1"]/tbody/tr[1]/td[2]/a')
                WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
                locator = (
                    By.XPATH, "//*[@id='zbjgmore2_DataGrid1']/tbody/tr[1]/td[2]/a[not(contains(string(),'%s'))]" % val)
                WebDriverWait(driver, 20).until(EC.presence_of_element_located(locator))
            page = driver.page_source
            body = etree.HTML(page)
            content_list = body.xpath('//*[@id="zbjgmore2_DataGrid1"]/tbody/tr')
        data = []
        for content in content_list:
            name = content.xpath("./td/a/text()")[0].strip()
            new_url = "http://www.lnzb.cn" + content.xpath("./td/a/@href")[0].strip()
            ggstart_time = content.xpath("./td[3]/text()")[0].strip()
            temp = [name, ggstart_time, new_url]
            # print(temp)
            data.append(temp)
        df = pd.DataFrame(data=data)
        df["info"] = None
        return df

def f2(driver):
    if "http://tlggzyjy.com" in driver.current_url:
        try:
            locator = (By.XPATH,"//div[@class='page_pagding']/div/span")
            WebDriverWait(driver,20).until(EC.presence_of_element_located(locator))
            total_page = driver.find_element_by_xpath("//div[@class='page_pagding']/div/span[last()]").text
        except:
            total_page = 1
        # return total_page
    else:
        if "00300100" in driver.current_url:
            locator = (By.ID, "zbggmore2_Pager")
            WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
            total_page = driver.find_element_by_xpath('//*[@id="zbggmore2_Pager"]/table/tbody/tr/td[1]/font[2]/b').text
        elif "00300200" in driver.current_url:
            locator = (By.ID, "zbgsmore2_Pager")
            WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
            total_page = driver.find_element_by_xpath('//*[@id="zbgsmore2_Pager"]/table/tbody/tr/td[1]/font[2]/b').text
        elif "00300300" in driver.current_url:
            locator = (By.ID, "zbjgmore2_Pager")
            WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
            total_page = driver.find_element_by_xpath('//*[@id="zbjgmore2_Pager"]/table/tbody/tr/td[1]/font[2]/b').text

    driver.quit()
    return int(total_page)

def f3(driver, url):
    driver.get(url)
    if "http://tlggzyjy.com" in driver.current_url:
        locator = (By.CLASS_NAME, "page_r_info")
        WebDriverWait(driver, 10).until(EC.presence_of_all_elements_located(locator))
        before = len(driver.page_source)
        time.sleep(0.1)
        after = len(driver.page_source)
        i = 0
        while before != after:
            before = len(driver.page_source)
            time.sleep(0.1)
            after = len(driver.page_source)
            i += 1
            if i > 5: break
        page = driver.page_source
        soup = BeautifulSoup(page, 'lxml')
        div = soup.find("div",class_="page_r_info")
        return div

    else:
        locator = (By.ID, "tblInfo")
        WebDriverWait(driver, 10).until(EC.presence_of_all_elements_located(locator))
        before = len(driver.page_source)
        time.sleep(0.1)
        after = len(driver.page_source)
        i = 0
        while before != after:
            before = len(driver.page_source)
            time.sleep(0.1)
            after = len(driver.page_source)
            i += 1
            if i > 5: break
        page = driver.page_source
        soup = BeautifulSoup(page, 'lxml')
        div = soup.find('table', id='tblInfo')
        return div

data = [
    ["zfcg_zhaobiao_tielingshi_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=1&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"],f1, f2],
    ["zfcg_zhaobiao_tielingxian_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=2&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhaobiao_kaiyuanshi_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=3&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhaobiao_changtuxian_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=4&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhaobiao_xifengxian_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=5&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhaobiao_diaobingshanshi_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=6&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhaobiao_yinzhou_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=7&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhaobiao_qinghequ_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=8&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhaobiao_kaifaqu_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=9&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],



    ["zfcg_zhongbiao_tielingshi_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=3&CountyId=1&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhongbiao_tielingxian_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=3&CountyId=2&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhongbiao_changtuxian_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=3&CountyId=4&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhongbiao_diaobingshanshi_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=3&CountyId=6&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhongbiao_yinzhou_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=3&CountyId=7&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["zfcg_zhongbiao_kaifaqu_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=3&CountyId=9&TradeTypeId=1",
     ["name", "ggstart_time", "href", "info"], f1, f2],

    #gcjs _ zhaobiao
    ["gcjs_zhaobiao_tielingshi_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=1&TradeTypeId=2",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["gcjs_zhaobiao_tielingxian_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=2&TradeTypeId=2",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["gcjs_zhaobiao_kaiyuanshi_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=3&TradeTypeId=2",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["gcjs_zhaobiao_changtuxian_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=4&TradeTypeId=2",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["gcjs_zhaobiao_xifengxian_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=5&TradeTypeId=2",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["gcjs_zhaobiao_diaobingshanshi_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=6&TradeTypeId=2",
     ["name", "ggstart_time", "href", "info"], f1, f2],
    ["gcjs_zhaobiao_yinzhou_gg",
     "http://tlggzyjy.com/Project/Index/1?ProjectState=1&CountyId=7&TradeTypeId=2",
     ["name", "ggstart_time", "href", "info"], f1, f2],

]


def work(conp,**args):
    est_meta(conp, data=data, diqu="辽宁省铁岭市",**args)
    est_html(conp, f=f3,**args)


if __name__ == "__main__":
    conp=["postgres", "since2015", "192.168.3.171", "liaoning", "tieling"]
    import sys
    arg=sys.argv
    if len(arg) >3:
        work(conp,num=int(arg[1]),total=int(arg[2]),html_total=int(arg[3]))
    elif len(arg) == 2:
        work(conp, html_total=int(arg[1]))
    else:
        work(conp)