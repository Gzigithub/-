...
_name_='dandong'
def f1(driver, num):
    if "http://ddpas.dandong.gov.cn" in driver.current_url:
        try:
        ...
        if int(cnum) != int(num):
            locator = (By.CLASS_NAME,"ewb-page-input")
            WebDriverWait(driver,20).until(EC.visibility_of_element_located(locator))
            driver.find_element_by_class_name("ewb-page-input").clear()
            driver.find_element_by_class_name("ewb-page-input").send_keys(num)
            driver.find_element_by_class_name("ewb-page-go").click()
            locator = (By.XPATH, "//ul[@class='ewb-list']/li[1]/a")
            WebDriverWait(driver, 20).until(EC.visibility_of_element_located(locator))
            locator = (
                By.XPATH, "//ul[@class='ewb-list']/li[1]/a[not(contains(@href,'%s'))]" % val)
...
def f2(driver):
    if "http://ddpas.dandong.gov.cn" in driver.current_url:
        ...
        for i in range(2,10000):
            next_url = '/'.join(url.split('/')[:-1]) + '/' +  str(i) +".html"
...
if __name__ == "__main__":
    conp=["postgres", "since2015", "192.168.3.171", "liaoning", "dandong"]
    import sys
    arg=sys.argv
    if len(arg) >3:
