...
def f1(driver, num):
        ...
        if num <= 10 and int(cnum) <= 10:
            ...
            if time:
                for i in range(time):
                    if i == 0:
                        if int(cnum) < 11:
                            locator = (By.ID, "Main_unitList_unitPager_lstMain_btnPage_10")
                            WebDriverWait(driver, 30).until(EC.visibility_of_element_located(locator))
                            driver.find_element_by_id("Main_unitList_unitPager_lstMain_btnPage_10").click()
                        else:
    ...
    # cnum2 = driver.find_element_by_xpath(
    #     "//input[@class='pager-content-selected' and @style='width:20px;cursor:hand;']").get_attribute("value")
    # print("起始页：", cnum, "当前页：", cnum2, "目标页：", num)
